...
const updateTrainer = catchAsync(async (req: Request, res: Response) => {
  const user: IUser = await TrainerService.updateTrainer(
...
const getTrainer = catchAsync(async (req: Request, res: Response) => {
  const user: IUser | null = await TrainerService.getTrainer(
    Number(req.params.id)
  );
...
const getOneDayScheduleByTrainer = catchAsync(
  async (req: Request, res: Response) => {
    const schedules = await TrainerService.getOneDayScheduleByTrainer(
      Number(req.params.id)
    );
